...
(() => {
 ...
 function openPuzzle(id){
  ...
  const steps=G.steps?.[id];const stepsSeen=state.seen.includes('steps:'+id);const text=G.texts?.[id];open(p.title,`${text?readingHtml(text,true):''}<div class="puzzle-head"><p>${esc(p.prompt)}</p><div class="puzzle-help-buttons">${text?'<button type="button" id="puzzle-text" aria-expanded="false">📜 Fachtext</button>':''}<button id="puzzle-hint" aria-label="Hinweis zum Rätsel">♧ Hinweis</button></div></div>${steps?`<details class="puzzle-steps"${has(id)||state.seen.includes('steps:'+id)?'':' open'}><summary>So funktioniert's</summary><ol>${steps.map(t=>`<li>${esc(t)}</li>`).join('')}</ol></details>`:''}<p id="hint-box" class="clue" hidden></p><div id="puzzle-work" class="${p.type}"></div><div id="feedback" role="status" aria-live="polite"></div>`,'Erinnerung · '+scene().era,'puzzle');activePuzzle=id;$('#puzzle-hint').onclick=hint;if(steps&&!stepsSeen){add('seen','steps:'+id);save();}if(text){const mc=$('#modal-content');const setTab=t=>{mc.dataset.tab=t;$('#puzzle-text')?.setAttribute('aria-expanded',String(t==='read'));if(t==='read'){add('seen','text:'+id);save();}$('#modal').scrollTop=0;};$('#puzzle-text').onclick=()=>setTab('read');mc.querySelector('.to-puzzle').onclick=()=>setTab('solve');setTab('solve');}
