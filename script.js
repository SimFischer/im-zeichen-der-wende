...
(() => {
 ...
 function readArchiveScrolls(){
  ...
  function show(){open('Die Schriftrollen des Archivars',`<div class="scroll-view"><article class="scroll-paper s${page%3+1}" aria-label="Schriftrolle ${page+1} von ${t.body.length}"><img class="scroll-art" src="assets/puzzles/archive/schriftrolle-grosse-textflaeche.png" alt="" draggable="false"><div class="scroll-safe"><h3>${esc(t.title)}</h3><p>${esc(t.body[page])}</p><span class="scroll-count">Schriftrolle ${page+1} von ${t.body.length}</span></div></article><nav class="scroll-nav" aria-label="Schriftrollen blättern"></nav></div>`,'Auftrag für das Archiv','scroll');const a=$('.scroll-nav');
 ...
 function showEnding(replay=true){if(!window.Finale||!canFinish())return false;const m=$('#modal');if(m.open){window.MiniGames?.stop?.();m.close();}activePuzzle=null;state.flags.finished=true;save();
  const el=window.Finale.play({names:G.seals,seals:state.seals,onExplore:()=>render(),onNewGame:()=>reset(()=>{close();showEnding(false);}),onJournal:()=>showJournal(),onEnd:()=>{state.flags.finaleSeen=true;save();}});if(el&&!replay)el.querySelector('.fn-skip')?.click();return true;}
 ...
 function menu(){activePuzzle=null;open('Im Zeichen der Wende',`<p class="eyebrow">Ein historisches Point-and-Click-Adventure</p><p class="intro-copy">Eine verschlossene Chronik. Sechs fehlende Siegel. Und eine Stadt, deren Geschichte sich grundlegend verändert.</p><p>Du bist Bote oder Botin. Untersuche Gegenstände, sprich mit Menschen und verbinde ihre Spuren. Stadtkarte, Botenbeutel und Notizbuch begleiten dich.</p>${!storageOK?'<p class="save-warning">Speichern ist gerade nicht verfügbar. Lass diesen Tab geöffnet.</p>':''}`,'Willkommen','menu');const a=actions();button(state.started?'Spiel fortsetzen':'Die Stadt betreten',()=>{state.started=true;save();close();render();if(state.flags.finished&&showEnding(false))return;if(!state.seen.includes('intro:gate')){add('seen','intro:gate');save();intro(G.scenes[0]);}},'primary',a);if(state.started)button('Neues Spiel',reset,'',a);button('Spielstand speichern / Code laden',continuationMenu,'',a);button('So spielst du',()=>info('So spielst du','Grüne Markierungen (i) informieren: Sprich mit den Menschen und untersuche die Dinge. Rätsel sind terrakottafarben (✦) – sie öffnen sich erst, wenn du dort genug erfahren hast (🔒 zeigt, wie viel noch fehlt). Beige Namensschilder kennzeichnen Gegenstände zum Mitnehmen. Bonusspiele schaltest du durch gelöste Rätsel frei und startest sie im Notizbuch. Kombinieren: Gegenstand wählen, dann Ziel antippen. Die Öllampe gibt drei gestufte Hilfen.'),'',a);}
