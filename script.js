...
(() => {
 ...
 function open(title,html,kicker='Die Stadtchronik',mode='info') {returnFocus=document.activeElement;modalMode=mode;$('#modal-title').textContent=title;$('#modal-kicker').textContent=kicker;$('#modal-content').innerHTML=html;if(!$('#modal').open)$('#modal').showModal();$('#modal').scrollTop=0;$('#close').focus();}
 ...
 function render(){unlock();const s=scene();$('#scene-name').textContent=s.name;$('#era').textContent=s.era;
  ...
  $('#objective').textContent=objective();renderInventory();save();
 }
 ...
 function interact(h,i){add('seen',state.scene+':'+i);save();const [label,x,y,type,id]=h;
  ...
  if(type==='talk'){const t=G.talks[id];info(t[0],t[1]);return;}
  ...
  if(type==='finalgate'){if(state.seals.length<6){info('Die Mechanik wartet','Noch fehlen Erkenntnis-Siegel. Erkunde die offenen Orte auf der Stadtkarte.');return;}state.flags.sealsPlaced=true;save();render();info('Die sechs Siegel greifen ineinander','Konflikt, Quelle, Anzeige, Staat, 312 und Wende: Die Zeitmechanik ist frei. Ordne zuerst die Ereignisse; danach kannst du die Argumentationsbrücke bauen.');return;}
 ...
 function selectItem(id){if(selected===id){selected=null;renderInventory();return;}
  ...
  selected=id;renderInventory();toast(G.items[id]+' gewählt. Tippe jetzt das Ziel an.');
 }
 ...
 function showMap(){activePuzzle=null;open('Wege durch die Erinnerungen','<p>Jeder Ort zeigt eine andere Zeit. Du kannst zu geöffneten Orten zurückkehren.</p><div class="map-grid"></div>','Stadtkarte','map');
  G.scenes.forEach(s=>{const accessible=state.unlocked.includes(s.id),b=button(s.name,()=>{
   ...
   enter(s.id);
  },'map-place',$('.map-grid'));b.disabled=!accessible;b.innerHTML=`<strong>${esc(s.name)}</strong><small>${accessible?(s.id===state.scene?'Du bist hier':esc(s.era)):'Noch nicht zugänglich · weitere Spuren finden'}</small>`;});
 }
 $('#map').onclick=showMap;
