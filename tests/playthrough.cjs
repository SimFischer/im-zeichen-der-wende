...
function boot(raw,opts={}){if(raw!==undefined)storage.set(KEY,raw);const {window}=parseHTML(fs.readFileSync(path.join(root,'index.html'),'utf8'));const document=window.document;
 ...
 for(const f of ['data/game-data.js','adventure.js','seals.js','bonusgames.js',...fs.readdirSync(path.join(root,'bonus')).filter(f=>f.endsWith('.js')).map(f=>'bonus/'+f),'script.js'])vm.runInContext(fs.readFileSync(path.join(root,f),'utf8'),context,{filename:f});
