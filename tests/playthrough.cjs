...
function boot(raw,opts={}){if(raw!==undefined)storage.set(KEY,raw);const {window}=parseHTML(fs.readFileSync(path.join(root,'index.html'),'utf8'));const document=window.document;
 ...
 for(const f of ['data/game-data.js','adventure.js','bonusgames.js',...fs.readdirSync(path.join(root,'bonus')).filter(f=>f.endsWith('.js')).map(f=>'bonus/'+f),'script.js'])vm.runInContext(fs.readFileSync(path.join(root,f),'utf8'),context,{filename:f});
...
function travel(id){click('#map');const name=env.window.GAME.scenes.find(s=>s.id===id).name;const b=all('.pm-place').find(n=>n.querySelector('.pm-label b').textContent===name);ok(b&&!b.disabled,'Reachable '+id);b.click();if($('#modal').open&&$('#modal-title').textContent===name)close();ok(state().scene===id,'Scene entered '+id);}
...
function solve(id){const p=env.window.GAME.puzzles[id],mg=env.window.GAME.minigames?.[id];
 ...
 textButton(p.type==='balance'?'Einordnung reflektieren':'Mechanismus prüfen');if(p.type==='balance')textButton('Ich habe meine Begründung geprüft – Siegel nehmen');ok(state().solved.includes(id),'Solved '+id);close();}
...
travel('city');explore();spot('change');ok(!env.window.GAME.minigames.change,'No minigame registered for change');ok($('#modal').dataset.mode!=='minigame'&&$('#modal-title').textContent==='Die Besitztruhe','Besitztruhe opens the classic puzzle');ok(!env.document.body.innerHTML.includes('Kurierfahrt'),'Removed courier game is not shown');solve('change');travel('motives');explore();spot('motives');solve('motives');ok(state().seals.length===6,'Six unique seals');travel('council');explore();spot('council');solve('council');travel('basilica');spot('timeline');ok($('#modal-title').textContent==='Hier fehlt noch etwas','Final requires placed seals');close();spot('finalgate');for(const seal of env.window.GAME.seals){textButton(seal,'.seal-rack');textButton(seal,'.seal-wheel');}close();spot('bridge');ok($('#modal-title').textContent==='Hier fehlt noch etwas','Bridge gated by timeline');close();spot('timeline');solve('timeline');spot('bridge');solve('bridge');ok(state().flags.finished,'Finale reached naturally');ok(state().solved.length===12,'All 12 puzzles solved');
click('#notebook');ok($('#modal-content').textContent.includes('Deine Begründung'),'Own reasoning in notebook');ok($('#modal-content').textContent.includes('Freigeschaltet: 7 / 7'),'Notebook lists all bonus games');ok(all('.bonus-replay').length===7&&!all('.bonus-unknown').length,'All eight bonus games unlocked after the finale');close();
...
// Alte Spielstände aus der Zeit mit der Kurierfahrt: fremde Felder werden ignoriert, die Besitztruhe öffnet das klassische Rätsel.
storage.set('im-zeichen-der-wende:bonus-v1',JSON.stringify({found:['kurierfahrt','wagen'],won:['kurierfahrt','wagen']}));env=boot(JSON.stringify({version:1,started:true,scene:'city',unlocked:env.window.GAME.scenes.map(x=>x.id),solved:['conflict'],seals:[],inventory:[],notes:[],seen:[],evidence:[],hints:{change:1},drafts:{change:{values:[null,null,null,null,null],reason:''}},flags:{courierDone:true},courier:{best:6,difficulty:'Schnell'}}));textButton('Spiel fortsetzen');ok(state().scene==='city','Legacy save resumes');explore();spot('change');ok($('#modal-title').textContent==='Die Besitztruhe','Legacy save opens classic Besitztruhe');close();click('#notebook');ok(!$('#modal-content').textContent.includes('Kurierfahrt'),'Notebook without courier entry');ok(!$('#modal-content').textContent.includes('Belade den Wagen'),'Notebook without wagon entry');ok(all('.bonus-replay').length===1,'Unknown legacy bonus ids are ignored');close();ok(!JSON.parse(storage.get('im-zeichen-der-wende:bonus-v1')).won.includes('kurierfahrt'),'Legacy bonus id dropped on update');ok(!JSON.parse(storage.get('im-zeichen-der-wende:bonus-v1')).won.includes('wagen'),'Legacy wagen status dropped');
