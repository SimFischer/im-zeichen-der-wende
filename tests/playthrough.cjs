...
function boot(raw,opts={}){if(raw!==undefined)storage.set(KEY,raw);const {window}=parseHTML(fs.readFileSync(path.join(root,'index.html'),'utf8'));const document=window.document;
 ...
 window.MiniGames=new Proxy({stop(){}},{get:(target,type)=>target[type]||((id,cfg,work)=>{work.innerHTML=`<p class="stub-minigame" data-type="${String(type)}">${cfg.title}</p>`;return true;})});
 ...
 for(const f of ['data/game-data.js','adventure.js','chronicle.js','seals.js','bonusgames.js',...fs.readdirSync(path.join(root,'bonus')).filter(f=>f.endsWith('.js')).map(f=>'bonus/'+f),'script.js'])vm.runInContext(fs.readFileSync(path.join(root,f),'utf8'),context,{filename:f});
...
function solve(id){const p=env.window.GAME.puzzles[id],mg=env.window.GAME.minigames?.[id];
 if(mg&&id!=='timeline'){ok($('#modal').dataset.mode==='minigame','Minigame opens for '+id);ok($('#modal-title').textContent===mg.title,'Minigame title '+id);ok(!all('#modal-content button').some(b=>b.textContent==='Ohne Spiel lösen'),'No skip-the-game button for '+id);
 ...
 const checkLabel=p.type==='mosaic'?'Einordnung prüfen':'Mechanismus prüfen';
 if(p.reasons){
 ...
 textButton(checkLabel);if(id==='timeline'){env.timers.findLast(t=>t.t===1800).f();click('[data-gap="0"]');textButton('Zur Argumentationsbrücke');}if(p.type==='mosaic')textButton('Verstanden – Siegel nehmen');ok(state().solved.includes(id),'Solved '+id);close();}
...
ok(state().flags.sealsPlaced&&$('.seal-chamber').classList.contains('complete'),'All six seals engage');textButton('Zur Zeitmechanik');spot('bridge');ok($('#modal-title').textContent==='Hier fehlt noch etwas','Bridge gated by timeline');close();spot('timeline');ok(!!$('.chronicle-machine'),'Chronikmaschine öffnet sich');solve('timeline');spot('bridge');ok($('#modal-content .stub-minigame').dataset.type==='argbridge','Argumentationsbrücke öffnet die neue Brückenszene');solve('bridge');ok(state().flags.finished,'Finale reached naturally');ok(state().solved.length===12,'All 12 puzzles solved');textButton('Stadt weiter erkunden');
...
env=boot();textButton('Spiel fortsetzen');ok(state().flags.finished&&state().seals.length===6,'Completed save reload');ok(!$('.finale-end').hidden,'Completed save shows ending');textButton('Stadt weiter erkunden');
// Alte Spielstände mit Freitext bleiben gültig: kein Fortschritt geht verloren, der Text erscheint als frühere Notiz.
{const old=state();old.drafts.motives.reason='Mein alter Freitext aus der früheren Version.';env=boot(JSON.stringify(old));textButton('Spiel fortsetzen');ok(state().solved.includes('motives')&&state().seals.length===6,'Legacy free-text save keeps progress');textButton('Stadt weiter erkunden');click('#notebook');ok($('#modal-content').textContent.includes('Deine frühere Notiz'),'Legacy text shown as earlier note');close();}
