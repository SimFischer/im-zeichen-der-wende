...
click('#notebook');ok($('#modal-content').textContent.includes('Deine gewählte Begründung'),'Chosen reasoning in notebook');ok($('#modal-content').textContent.includes('Freigeschaltet: 9 / 9'),'Notebook lists all bonus games');ok(all('.bonus-replay').length===9&&!all('.bonus-unknown').length,'All nine bonus games unlocked after the finale');close();
