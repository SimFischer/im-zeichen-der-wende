...
click('#notebook');ok($('#modal-content').textContent.includes('Deine Begründung'),'Own reasoning in notebook');ok($('#modal-content').textContent.includes('Freigeschaltet: 8 / 8'),'Notebook lists all bonus games');ok(all('.bonus-replay').length===8&&!all('.bonus-unknown').length,'All eight bonus games unlocked after the finale');close();
