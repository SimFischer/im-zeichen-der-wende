...
const milestones=['conflict','sources','cases','archive','map312','vision','change','motives','bridge'];
const games=['zeichen','rombrennt','amphoren','katakomben','schildwall','tiber','wagen','kurier','circus'];
for(let n=0;n<=9;n++){
 ...
 assert.equal(journal.querySelectorAll('.bonus-unknown').length,9-n);
...
B.reset();B.update({solved:[]});journal.innerHTML=B.journalHtml();assert.equal(journal.querySelectorAll('.bonus-unknown').length,9);
assert.equal(B.decorate,undefined,'No scene discovery entry point');
console.log('PASS: nine milestones, locked start protection, journal launch and fresh-game reset');
