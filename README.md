...
Drei vom Nutzer bereitgestellte Spielgrafiken sind enthalten. `scene-gate.png` und `scene-city-313.png` sind Einzelszenen; `scene-atlas.png` enthält neun Szenen, die CSS als Ausschnitte darstellt. Die Beratung von Nicäa verwendet vorläufig denselben Hintergrund wie die Motivwaage. Die Atlas-Ausschnitte haben geringere Detailauflösung als die Einzelbilder. Illustrationen sind keine historischen Belegbilder.

Texte, Rätsel, Dialoge, Hilfen, akzeptierte Antworten und fachliches Feedback: `data/game-data.js`. Mechanik, Speicherlogik und globale Bedienungstexte: `script.js`. Darstellung: `style.css`. Eine Zeile eines Rätsels enthält Optionen, akzeptierte Antwortindizes und erklärendes Feedback. Bei inhaltlichen Änderungen Lösungsschlüssel und Hilfen zusammen pflegen.
